...
:doc-url: https://documentation.bonitasoft.com/bonita/10.0
:java-version: 1.8
= connector-kafka-producer

The project **connector-kafka-producer** is a Bonita Connector for **Bonita {short-bonita-version}**  **Community** version written in `java` language.
...
[source,bash]
----
git clone https://your.github.com/connector-kafka-producer.git
cd connector-kafka-producer/
./mwnw package
----

The build should produce a zip archive under the `target/` folder named `connector-kafka-producer-10.0.0-all.zip`
...
Take the built artifact `connector-kafka-producer-10.0.0-all.zip` and import it to your Bonita Studio.
